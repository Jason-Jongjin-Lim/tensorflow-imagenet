...
flags.DEFINE_enum('variable_update', 'parameter_server',
                  ('parameter_server', 'replicated', 'distributed_replicated',
                   ...
                   'collective_all_reduce', 'perseus', 'horovod'),
                  'The method for managing variables: parameter_server, '
                  'replicated, distributed_replicated, independent, '
                  'distributed_all_reduce, collective_all_reduce, perseus, horovod')
...
def create_config_proto(params):
  ...
  if params.variable_update == 'perseus':
    import perseus.tensorflow.horovod as hvd  # pylint: disable=g-import-not-at-top
    config.gpu_options.visible_device_list = str(hvd.local_rank())
  elif params.variable_update == 'horovod':
...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    elif self.params.variable_update == 'perseus':
      import perseus.tensorflow.horovod as hvd  # pylint: disable=g-import-not-at-top
    ...
    elif self.params.variable_update in ('independent', 'perseus', 'horovod'):
      if self.job_name:
        raise ValueError('Invalid variable_update in distributed mode: %s' %
                         self.params.variable_update)
  ...
  def print_info(self):
    ...
    if self.params.variable_update in ['perseus', 'horovod'] and self.params.horovod_device:
  ...
  def _get_params_info(self):
    ...
    elif self.params.variable_update in ['perseus', 'horovod']:
  ...
  def _benchmark_graph(self, graph_info, eval_graph_info):
    ...
    if self.params.variable_update == 'perseus':
      import perseus.tensorflow.horovod as hvd  # pylint: disable=g-import-not-at-top
    ...
    sv = tf.train.Supervisor(
        ...
        # workers.
        is_chief=is_chief or (self.params.variable_update in ['perseus', 'horovod']
  ...
  def _build_fetches(self, global_step, all_logits, losses, device_grads,
    ...
    if (self.params.variable_update in ['perseus', 'horovod'] or
